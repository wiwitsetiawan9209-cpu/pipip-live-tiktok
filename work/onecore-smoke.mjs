...
var WindowsOneCoreTtsProvider = class {
  ...
  async listVoices() {
    ...
    try {
      ...
      const voices = parseJson(r.stdout)?.voices;
      if (r.code !== 0 || !Array.isArray(voices)) return [];
      this.voiceCache = voices.filter((v) => Boolean(v && typeof v.id === "string" && typeof v.name === "string" && typeof v.language === "string")).map((v) => ({ ...v, provider: this.id, locale: v.locale ?? v.language, available: true, capabilities: ["windows-onecore", "wav"] }));
  ...
  async getCapabilities() {
    ...
    return { providerId: this.id, local: true, languages: [...new Set(voices.map((v) => v.language))], formats: ["wav"] };
  }
  async stop() {
  ...
  async synthesize(request, signal) {
    ...
    const voices = await this.listVoices();
    const candidates = voices.filter((v) => normalizeLanguage(v.language) === language);
